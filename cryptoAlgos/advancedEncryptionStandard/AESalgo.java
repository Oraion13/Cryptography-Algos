...
public class AESalgo {
        ...
        for (int i = 0; i < storage.length; i++) {
            for (int j = 0; j < storage[0].length; j++) {
                for (int k = 0; k < MSG[0].length; k++) {
                    ...
                    //if the matrix's element(4 x 1 matrix) is 1 then no need to multiply, because(1 x n = n)
                    if(FIXED[i][k] != 0x01){
                        ...
                        int rslt = L_TABLE[Integer.parseInt(fstr.substring(0, 1), 16)][Integer.parseInt(fstr.substring(1), 16)]//FixedMatrix Element for Index in L_TABLE
                                                                      +
                                     L_TABLE[Integer.parseInt(MSG[k][j].substring(0, 1), 16)][Integer.parseInt(MSG[k][j].substring(1), 16)];//MSGMatrix Element for Index in L
    ...
    private static String Coder(String[][] MSG, int count, String option) {
            ...
            for (int i = 0; i < MSG.length; i++) {
                for (int j = 0; j < MSG.length; j++) {
                    MSG[i][j] = Integer.toHexString(
                            Integer.parseInt(MSG[i][j], 16) ^ Integer.parseInt(KEYGEN[(option == "ENCODE") ? (count) : (count)][i][j], 16));
                    if (MSG[i][j].length() != 2) {
                        MSG[i][j] = "0" + MSG[i][j];
                    }
                    System.out.print(MSG[i][j] + " ");
        ...
        if(option == "ENCODE"){
            return Coder(MSG, ++count, "ENCODE");
        }else{
            return Coder(MSG, --count, "DECODE");
        }
    ...
    public static void main(String[] args) throws UnsupportedEncodingException {
        String keyHex = toHex("Thats my Kung Fu");// Thats my Kung Fu
        ...
        String cipher = Coder(MSG, 1, "ENCODE");// 29c3505f571420f6402299b31a02d73a
        System.out.println("\n******Encrypted Hex: " + cipher + " ******\n");

        String[][] CIPHER = firstMat(cipher, "MESSAGE", "DE");
        String decoded = Coder(CIPHER, 9, "DECODE");
